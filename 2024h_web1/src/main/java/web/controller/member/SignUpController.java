...
public class SignUpController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 11. 첨부파일 아닌 일반 텍스트/값 dto로 직접파싱 
		MemberDto memberDto = new MemberDto();
		memberDto.setMid(fileList.get(0).getString()); // fileList.get(0).getString() : 첫번째 필드의 텍스트 가져오기
		memberDto.setMpwd(fileList.get(1).getString());	// 첫번째 필드의 텍스트/값 가져오기
		memberDto.setMname(fileList.get(2).getString()); // 두번째 필드의 텍스트/값 가져오기	
		memberDto.setMphone(fileList.get(3).getString()); // 세번째 필드의 텍스트/값 가져오기
