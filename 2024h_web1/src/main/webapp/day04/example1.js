...
//2. JS함수
// (1) 선언적 함수 : fuction 함수명(매개변수명, 매개변수명){실행문;}
// (정의 / 함수만들기)
...
console.log(result);
//(2) 익명 함수 : fuction(매개변수명 , 매개변수명){실행문;}
...
//(3) 배열변수명.map((반복변수명) => {실행문}) 함수, 배열내 요소를 하나씩 반복변수명에 대입 반복
words.map( (word) => {console.log(word)} );

//(*) forEach 와 map 함수의 차이점 : firEach함수는 return이 없고, map함수는 return지원한다.
let newWords1 = words.forEach((i) => {return i;});
console.log(nweWords1); //ReferenceError: nweWords1 is not defined
